...
class CBR_custom_blog extends ET_Builder_Module_Type_PostBased {
	...
	function init() {
		$this->name             = esc_html__('Custom Blog Module', 'et_builder');
		$this->plural           = esc_html__('Custom Blogs Module', 'et_builder');
	...
	static function rename_meta_fields() {
	...
	function get_fields() {
		...
		$fields = array(
			...
			'subtitle_meta_field'  => array(
				'label'            => esc_html__('Subtitle Meta Field', 'myex-my-extension'),
				'type'             => 'select',
				'option_category'  => 'basic_option',
				'options'          => $posts_meta_fields,
				'toggle_slug'      => 'main_content',
				'description'      => esc_html__('Choose the subtitle meta field', 'myex-my-extension'),
				'computed_affects' => array(
					'__posts',
				),
				'show_if'          => array(
			...
			'include_tags'            => array(
				'label'            => esc_html__('Included Tags', 'myex-my-extension'),
				'type'             => 'categories',
				'meta_categories'  => array(
				...
				'description'      => esc_html__('Choose which tags you would like to include in the feed.', 'myex-my-extension'),
				'toggle_slug'      => 'main_content',
				'computed_affects' => array(
					'__posts',
				),
			...
			'show_tags'               => array(
				...
				'description'      => esc_html__('Turn the tags links on or off.', 'et_builder'),
				'computed_affects' => array(
					'__posts',
				),
				'toggle_slug'      => 'elements',
			...
			'__posts'                 => array(
				'type'                => 'computed',
				'computed_callback'   => array('CBR_custom_blog', 'get_blog_posts'),
	...
	public function get_custom_css_fields_config() {
		...
		$custom_css = array(
			'content' => array(
				'label'    => esc_html__('Meta Subtitle', 'simp-simple'),
				'selector' => '%%order_class%% .meta_subtitle',
			),
			'heading' => array(
			...
			'date_categories' => array(
				'label'    => esc_html__('Date & Categories', 'simp-simple'),
				'selector' => '%%order_class%% p.post-meta',
			),
			'more_link' => array(
				'label'    => esc_html__('More Link', 'simp-simple'),
	...
	static function get_blog_posts($args = array(), $conditional_tags = array(), $current_page = array()) {
		...
		if ($query->have_posts()) {
			...
			while ($query->have_posts()) {
				...
				$untertitel_meta_key = get_post_meta($post->ID, $args['subtitle_meta_field'], true) ? get_post_meta($post->ID, $args['subtitle_meta_field'], true) : "";
...
?>
				...
				<article id="" <?php post_class('et_pb_post clearfix' . $no_thumb_class . $overlay_class); ?>>
					...
					<?php if ('off' === $args['fullwidth'] || !in_array($post_format, array('link', 'audio', 'quote'))) { ?>
						<?php if (!in_array($post_format, array('link', 'audio'))) { ?>
							...
							/**
							 * Print Subtitle Meta Field
							 */
							//$data = get_post_meta($post->ID, $args['subtitle_meta_field'], true);
	...
	function render($attrs, $content = null, $render_slug) {
		...
		// Sending args to get_posts method
		CBR_custom_blog::get_blog_posts($this->props);
		...
		// Addded custom ordering of Posts
		//$args['orderby'] = 'title';
